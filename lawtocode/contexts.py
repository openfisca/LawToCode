...
class Ctx(conv.State):
    ...
    def __init__(self, req = None):
        if req is not None:
            self.req = req
            lawtocode_env = req.environ.get('law-to-code', {})
            for key in object.__getattribute__(self, 'env_keys'):
                value = lawtocode_env.get(key)
    ...
    def blank_req(self, path, environ = None, base_url = None, headers = None, POST = None, **kw):
        ...
        lawtocode_env = env.setdefault('law-to-code', {})
        for key in self.env_keys:
            value = getattr(self, key)
            if value is not None:
                lawtocode_env[key] = value
    ...
    def lang_del(self):
        del self._lang
        if self.req is not None and self.req.environ.get('law-to-code') is not None \
                and '_lang' in self.req.environ['law-to-code']:
            del self.req.environ['law-to-code']['_lang']

    def lang_get(self):
        if self._lang is None:
            self._lang = ['en-US', 'en']
            if self.req is not None:
                self.req.environ.setdefault('law-to-code', {})['_lang'] = self._lang
        return self._lang
    ...
    def lang_set(self, lang):
        ...
        if self._translator is not None:
            ...
            if self.req is not None and self.req.environ.get('law-to-code') is not None \
                    and '_translator' in self.req.environ['law-to-code']:
                del self.req.environ['law-to-code']['_translator']
    ...
    def node_del(self):
        del self._node
        if self.req is not None and self.req.environ.get('law-to-code') is not None \
                and '_node' in self.req.environ['law-to-code']:
            del self.req.environ['law-to-code']['_node']
    ...
    def node_set(self, node):
        self._node = node
        if self.req is not None:
            self.req.environ.setdefault('law-to-code', {})['_node'] = node
    ...
    def session_del(self):
        del self._session
        if self.req is not None and self.req.environ.get('law-to-code') is not None \
                and '_session' in self.req.environ['law-to-code']:
            del self.req.environ['law-to-code']['_session']
    ...
    def session_set(self, session):
        ...
        if self.req is not None:
            self.req.environ.setdefault('law-to-code', {})['_session'] = session
    ...
    def user_del(self):
        del self._user
        if self.req is not None and self.req.environ.get('law-to-code') is not None \
                and '_user' in self.req.environ['law-to-code']:
            del self.req.environ['law-to-code']['_user']
    ...
    def user_set(self, user):
        self._user = user
        if self.req is not None:
            self.req.environ.setdefault('law-to-code', {})['_user'] = user
