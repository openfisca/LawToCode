...
inputs_to_formula_data = conv.struct(
    dict(
...
def admin_delete(req):
    ...
    if not model.is_admin(ctx):
        return wsgihelpers.forbidden(ctx,
            ...
            message = ctx._("You can not delete a formula."),
            title = ctx._('Operation denied'),
            )

    if req.method == 'POST':
        formula.delete(ctx, safe = True)
        return wsgihelpers.redirect(ctx, location = model.Formula.get_admin_class_url(ctx))
    return templates.render(ctx, '/formulas/admin-delete.mako', formula = formula)
...
def admin_edit(req):
    ...
    if req.method == 'GET':
        ...
        inputs = extract_formula_inputs_from_params(ctx, req.POST)
        data, errors = inputs_to_formula_data(inputs, state = ctx)
        if errors is None:
        ...
        if errors is None:
            if model.Formula.find(
                ...
                errors = dict(email = ctx._('A formula with the same email already exists.'))
        if errors is None:
            formula.set_attributes(**data)
            formula.compute_words()
            formula.save(ctx, safe = True)

            # View formula.
            return wsgihelpers.redirect(ctx, location = formula.get_admin_url(ctx))
    return templates.render(ctx, '/formulas/admin-edit.mako', errors = errors, inputs = inputs, formula = formula)
...
#    model.is_admin(ctx, check = True)
    ...
    if errors is not None:
        return wsgihelpers.not_found(ctx, explanation = ctx._('Formula search error: {}').format(errors))
    ...
    cursor = model.Formula.find(criteria, as_class = collections.OrderedDict)
    ...
    formulas = cursor.skip(pager.first_item_index or 0).limit(pager.page_size)
    return templates.render(ctx, '/formulas/admin-index.mako', data = data, errors = errors, formulas = formulas,
...
def admin_new(req):
    ...
    if user is None:
        return wsgihelpers.unauthorized(ctx,
            ...
            message = ctx._("You can not create a formula."),
            title = ctx._('Operation denied'),
            )
    ...
    if req.method == 'GET':
        errors = None
        inputs = extract_formula_inputs_from_params(ctx)
    else:
        assert req.method == 'POST'
        inputs = extract_formula_inputs_from_params(ctx, req.POST)
        data, errors = inputs_to_formula_data(inputs, state = ctx)
        ...
        if errors is None:
            if model.Formula.find(
                ...
                errors = dict(full_name = ctx._('A formula with the same name already exists.'))
        if errors is None:
            formula.set_attributes(**data)
            formula.compute_words()
            formula.save(ctx, safe = True)
            ...
            # View formula.
            return wsgihelpers.redirect(ctx, location = formula.get_admin_url(ctx))
    return templates.render(ctx, '/formulas/admin-new.mako', errors = errors, inputs = inputs, formula = formula)
...
def admin_view(req):
    ...
    return templates.render(ctx, '/formulas/admin-view.mako', formula = formula)
...
def api1_index(req):
    ...
    cursor = model.Formula.get_collection().find(None, [])
    return wsgihelpers.respond_json(ctx,
        collections.OrderedDict(sorted(dict(
            apiVersion = '1.0',
            context = data['context'],
            ...
            url = req.url.decode('utf-8'),
            value = [
                formula_attributes['_id']
                for formula_attributes in cursor
                ],
            ).iteritems())),
        headers = headers,
        jsonp = data['callback'],
...
def api1_typeahead(req):
    ...
    cursor = model.Formula.get_collection().find(criteria, ['title'])
    return wsgihelpers.respond_json(ctx,
        [
            formula_attributes['title']
            for formula_attributes in cursor.limit(10)
            ],
        headers = headers,
        )


def extract_formula_inputs_from_params(ctx, params = None):
    if params is None:
...
def index(req):
    ...
    if data['sort'] == 'slug':
        ...
        cursor.sort([(data['sort'], pymongo.DESCENDING), ('slug', pymongo.ASCENDING)])
    formulas = cursor.skip(pager.first_item_index or 0).limit(pager.page_size)
    return templates.render(ctx, '/formulas/index.mako', data = data, errors = errors, formulas = formulas,
        inputs = inputs, pager = pager)
...
def route_admin(environ, start_response):
    ...
    formula, error = conv.pipe(
        conv.input_to_slug,
        conv.not_none,
        model.Formula.make_id_or_slug_or_words_to_instance(),
        )(req.urlvars.get('id_or_slug_or_words'), state = ctx)
    if error is not None:
        return wsgihelpers.not_found(ctx, explanation = ctx._('Formula Error: {}').format(error))(
    ...
    if error is not None:
        ...
        return wsgihelpers.respond_json(ctx,
            collections.OrderedDict(sorted(dict(
                ...
                error = collections.OrderedDict(sorted(dict(
                    code = 404,  # Not Found
                    message = ctx._('Formula Error: {}').format(error),
                    ).iteritems())),
                method = req.script_name,
                url = req.url.decode('utf-8'),
                ).iteritems())),
