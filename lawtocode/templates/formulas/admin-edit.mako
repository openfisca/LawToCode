...
<%def name="container_content()" filter="trim">
        <form action="${formula.get_admin_url(ctx, 'edit')}" method="post" role="form">
            <%self:hidden_fields/>
            <fieldset>
                <legend>${_(u'Edition of %s') % formula.get_title(ctx)}</legend>
                <%self:error_alert/>
                <%self:form_fields/>
...
${_(u'Edit')} - ${formula.get_title(ctx)} - ${parent.title_content()}
</%def>
