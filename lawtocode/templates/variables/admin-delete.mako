...
<%def name="container_content()" filter="trim">
        <h2>${_(u'Deletion of {}').format(variable.get_title(ctx))}</h2>
        <p class="confirm">${_(u"Are you sure that you want to delete this variable?")}</p>
        <form method="post" action="${variable.get_admin_url(ctx, 'delete')}">
            <%view:view_fields/>
...
${_(u'Delete')} - ${variable.get_title(ctx)} - ${parent.title_content()}
</%def>
